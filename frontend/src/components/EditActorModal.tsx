...
const EditActorModal = ({ open, handleClose, actor }: Iprops) => {
  ...
  const [ageInvalid, setAgeInvalid] = useState("");
  const [about, setAbout] = useState(actor.about);
  const [aboutInvalid, setAboutInvalid] = useState("");
  ...
  const validateForm = (): string => {
    ...
    if (!age) {
      setAgeInvalid(errors.age);
      return errors.age;
    } else {
      setAgeInvalid("");
    }
    if (!about) {
      setAboutInvalid(errors.about);
      return errors.about;
    } else {
      setAboutInvalid("");
    }
